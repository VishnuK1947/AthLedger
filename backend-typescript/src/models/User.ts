...
userSchema.methods.addRevenue = async function (amount: number): Promise<IUser> {
...
userSchema.methods.addPublicMetric = async function (performanceId: Types.ObjectId): Promise<IUser> {
...
userSchema.methods.removePublicMetric = async function (performanceId: Types.ObjectId): Promise<IUser> {
...
userSchema.pre<IUser>("save", async function (next) {
  if (this.isModified("email")) {
    const existingUser = await (this.constructor as IUserModel).findOne({ email: this.email });
